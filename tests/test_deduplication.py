...
def test_no_duplicates_passes_through() -> None:
    blocks = [ContextBlock(content="a"), ContextBlock(content="b")]
...
def test_exact_duplicate_collapsed() -> None:
    blocks = [ContextBlock(content="same"), ContextBlock(content="same")]
    kept, dropped = deduplicate_exact(blocks)
    assert len(kept) == 1
...
def test_different_whitespace_is_not_exact_duplicate() -> None:
    kept, dropped = deduplicate_exact(
        [ContextBlock(content="hello world"), ContextBlock(content="hello  world")]
    )
    assert len(kept) == 2
    assert dropped == []
...
def test_higher_score_representative_kept() -> None:
    low = ContextBlock(content="dup", semantic_score=0.2, block_id="low")
    high = ContextBlock(content="dup", semantic_score=0.9, block_id="high")
...
def test_required_representative_kept_over_higher_score() -> None:
    req = ContextBlock(content="dup", required=True, semantic_score=0.1, block_id="req")
    hi = ContextBlock(content="dup", semantic_score=0.99, block_id="hi")
...
def test_first_occurrence_kept_on_tie_and_order_preserved() -> None:
    blocks = [
        ContextBlock(content="x", block_id="x1"),
        ContextBlock(content="y", block_id="y1"),
        ContextBlock(content="x", block_id="x2"),  # tie with x1
    ]
    kept, dropped = deduplicate_exact(blocks)
