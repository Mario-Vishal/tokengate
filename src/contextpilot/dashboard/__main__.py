...
def _beacon_paths() -> tuple[str, str]:
    ...
    return str(base / "beacon.db"), str(base / "contextpilot_audits.db")
...
def _default_store_path() -> str:
    ...
    return audit_db if Path(audit_db).exists() else "contextpilot_audits.db"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="SQLite audit store path (default: Beacon app-data dir, or ./contextpilot_audits.db)",
    )
    ...
    print(f"ContextPilot Dashboard  http://{args.host}:{args.port}  (store: {store_path})")
