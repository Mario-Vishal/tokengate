"""CP-003 tests: ContextBlock validation, id derivation, serialization."""
...
def test_out_of_range_scores_rejected(field: str, bad: float) -> None:
    with pytest.raises(InvalidBlockError):
        ContextBlock(content="hi", **{field: bad})


def test_in_range_scores_accepted() -> None:
    b = ContextBlock(content="hi", semantic_score=0.0, keyword_score=1.0, final_score=0.5)
    assert b.semantic_score == 0.0


def test_negative_token_count_rejected() -> None:
    with pytest.raises(InvalidBlockError):
        ContextBlock(content="hi", token_count=-1)


# --- id derivation --------------------------------------------------------

def test_block_id_auto_derived_and_stable() -> None:
    a = ContextBlock(content="same content")
    b = ContextBlock(content="same content")
    assert a.block_id == b.block_id
    assert a.block_id.startswith("blk_")


def test_block_id_differs_by_content() -> None:
    assert ContextBlock(content="a").block_id != ContextBlock(content="b").block_id


def test_explicit_block_id_preserved() -> None:
    assert ContextBlock(content="x", block_id="my-id").block_id == "my-id"


# --- token counting -------------------------------------------------------

def test_ensure_token_count_uses_counter_and_caches() -> None:
    b = ContextBlock(content="one two three")
...
def test_round_trip_dict() -> None:
    ...
    restored = ContextBlock.from_dict(b.to_dict())
    assert restored.to_dict() == b.to_dict()


def test_from_dict_ignores_unknown_keys() -> None:
    b = ContextBlock.from_dict({"content": "hi", "bogus": 123})
    assert b.content == "hi"


def test_copy_with_overrides() -> None:
    b = ContextBlock(content="hi", block_id="id1")
