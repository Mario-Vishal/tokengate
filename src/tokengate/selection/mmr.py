...
def _relevance(blocks: list[TokenBlock]) -> list[float]:
    raw: list[float] = []
    for b in blocks:
...
def _similarity(a: TokenBlock, b: TokenBlock) -> float:
...
def mmr_select(
    blocks: list[TokenBlock], *, lambda_: float = 0.5, top_k: int | None = None
) -> list[TokenBlock]:
